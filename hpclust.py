# HPClust: High-Performance Hybrid Algorithm for Minimum Sum-of-Squares Clustering of Infinitely Tall Data
# Original paper:
# Ravil Mussabayev, Rustam Mussabayev. "High-Performance Hybrid Algorithm for Minimum Sum-of-Squares Clustering of Infinitely Tall Data". Mathematics 2024, 12, 1930.
...
# Sequential HPClust
@njit(parallel = False)
def hpclust_sequential(points, n_centers = 3, sample_size = 100, max_iter = 10000, t_max = 10.0, local_max_iters=300, local_tol=0.0001, n_candidates = 3, printing=False):
    n_points, n_features = points.shape
...
# HPClust with "Inner Parallelism":
...
@njit(parallel = True)
def hpclust_inner(points, n_centers = 3, sample_size = 100, max_iter = 10000, t_max = 10.0, local_max_iters=300, local_tol=0.0001, n_candidates = 3, printing=False):
    n_points, n_features = points.shape
    assert sample_size <= n_points
    if printing:
        with objmode:
...
# HPClust with "Competitive Parallelism":
...
@njit(parallel = True)
def hpclust_competitive(points, n_centers = 3, sample_size = 100, max_iter = 10000, t_max = 10.0, local_max_iters=300, local_tol=0.0001, n_candidates = 3, printing=False):
    with objmode(start_time = 'float64'):
        start_time = time.perf_counter()
    ...
    objectives = np.full((n_threads, max_iter), np.inf)
    ...
    objective_times = np.full((n_threads, max_iter), 0.0)
    best_times = np.full(n_threads, 0.0)
    best_n_iters = np.full(n_threads, 0)
    
    # # At first, initialize each worker's sample with same K-means++ seeding
    # sample = points[np.random.choice(n_points, sample_size, replace=False)]
    # solution = np.full((n_centers, n_features), np.nan)
    # center_inds, num_dists = kmeans_plus_plus(sample, solution, n_centers, n_candidates)
    # for i in prange(n_threads):
    #     centers[i] = sample[center_inds,:]
    
    for t in prange(n_threads):
        while (np.sum(n_iters) < max_iter or max_iter <= 0) and (running_time[t] < t_max or t_max <= 0.0):        
            sample = points[np.random.choice(n_points, sample_size, replace=False)]
...
# HPClust with "Cooperative Parallelism":
# Separate data samples are processed in parallel while each sample is clustered on a separate CPU core
# using the regular / sequential implementations of the K-means and K-means++ algorithms.
# At all subsequent iterations, each worker uses the best set of centroids among all workers obtained
# at previous iterations to initialize a new random data sample.
# This parallelization mode is called cooperative since the workers share information about the best solutions.
@njit(parallel = True)
def hpclust_cooperative(points, n_centers = 3, sample_size = 100, max_iter = 10000, t_max = 10.0, local_max_iters=300, local_tol=0.0001, n_candidates = 3, printing=False):
    with objmode(start_time = 'float64'):
        start_time = time.perf_counter()
...
# HPClust with 'Hybrid Parallelism (competitive + cooperative)': 
# This parallelization approach involves two consecutive phases: competitive and cooperative.
...
@njit(parallel = True)
def hpclust_hybrid(points, n_centers = 3, sample_size = 100, max_iter1 = 10000, max_iter2 = 10000, t_max1 = 10.0, t_max2 = 10.0, local_max_iters=300, local_tol=0.0001, n_candidates = 3, printing=False):
    with objmode(start_time = 'float64'):
        start_time = time.perf_counter()
